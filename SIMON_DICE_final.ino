...
void mostrar_fallos(int*, int*, int*);	// La función "mostrar_fallo" dirá en que botón has fallado y cual deberías haber pulsado.
...
void loop() {
	...
	// "flagfail"	variable entera me servirá para saber si hay algún fallo en la secuencia introducida por los botones. 
	...
	// En caso de perder o llegar al final de las rondas máximas.
...
void crear_enunciado(int *vect, int *x) /// Introduzco el vector del enunciado y el número de rondas.
{
	int i;
	for (i = 0; i < (*x); i++)
	{
...
void mostrar_fallos(int *x, int *y, int *num) ///Los vectores de los parámetros son el pulso y la secuencia. Los iré comparando para ver que valores son diferentes. Mostraré por pantalla los fallos.
{
	int i;
	for (i = 0; i < *num; i++)
	{
	...
	/// Muestro la secuencia de leds que debería haber pulsado.
...
///Con esta función se informa de si has acertado o has perdido. También te dice por el LCD cuantos fallos has cometido y por el serial especifica cuales.
...
/// Esta función sirve tanto si ganas o si pierdes. Si ganas te felicita, si pierdes te muestra los fallos que has tenido confunciones anteriores.
